...
This project is a contract-first example for ADR-001 through ADR-010. It
models an incident-response field console that synchronizes incidents,
captures field evidence, renders a cross-platform UI, and defines a signable
component build plan without embedding a payload or signature.
...
The delivered toolchain is `nexac 0.0.8`. Rust remains the default complete
Language 1.0 path; the Futao-written bootstrap path currently implements the
lexer and parser differentials. It must reject or fail to parse many files
...
target-2.0/src/           cohesive Futao 2.0 application sources
...
futao.lock                checked design lock fixture with real SHA-256 inputs
...
The two SHA-256 values in `futao.lock` are fixture integrity values, not output
from an implemented Futao resolver. The manifest digest hashes `futao.toml`.
The source-tree digest hashes the path-sorted `SHA-256 + relative path` records
for `futao.toml`, `release/`, and `target-2.0/`; `futao.lock` is excluded to
avoid a recursive digest. A future package implementation must replace this
fixture procedure with ADR-009 canonicalization.
