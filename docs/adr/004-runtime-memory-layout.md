...
[`PartialInit<T>` 有界部分初始化参考模型](../implementation/adr-004-partial-init.md)，以及
[NIR checked tagged-union target layout](../implementation/adr-004-tagged-union-layout.md)。这些切片不等同于
ADR-004 全部实现；规范化 layout table、Drop lowering、跨后端布局和稳定外部描述符仍按独立验收门槛推进。
